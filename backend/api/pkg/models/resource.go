...
func AddCatalogResource(db *gorm.DB, resource *Resource) (int, error) {
	sqlStatement := `
	INSERT INTO RESOURCE (NAME,DOWNLOADS,RATING,GITHUB)
	VALUES ($1, $2, $3, $4) RETURNING ID`
	var resourceID int
	err := db.DB().QueryRow(sqlStatement, resource.Name, resource.Downloads, resource.Rating, resource.Github).Scan(&resourceID)
	if err != nil {
		log.Println(err)
		return 0, err
	}
...
func AddResource(db *gorm.DB, resource *Resource, userID int, owner string, respositoryName string, path string) (int, error) {
	var resourceID int
	sqlStatement := `
	INSERT INTO RESOURCE (NAME,DESCRIPTION,DOWNLOADS,RATING,GITHUB,TYPE)
	VALUES ($1, $2, $3, $4, $5,$6) RETURNING ID`
	err := db.DB().QueryRow(sqlStatement, resource.Name, resource.Description, resource.Downloads, resource.Rating, resource.Github, resource.Type).Scan(&resourceID)
	if err != nil {
		return 0, err
	}
	// Add Tags separately
	if len(resource.Tags) > 0 {
		for _, tag := range resource.Tags {
			...
			err := db.DB().QueryRow(sqlStatement, tag).Scan(&tagID)
			if err != nil {
				tagExists = false
				log.Println(err)
			}
			...
			if tagExists {
				addResourceTag(db, resourceID, tagID)
			} else {
				var newTagID int
				newTagID, err = AddTag(db, tag)
				if err != nil {
					log.Println(err)
				}
				addResourceTag(db, resourceID, newTagID)
			}
		}
	}
	addGithubDetails(db, resourceID, owner, respositoryName, path)
	return resourceID, addUserResource(db, userID, resourceID)
}

func addResourceTag(db *gorm.DB, resourceID int, tagID int) {
	sqlStatement := `INSERT INTO RESOURCE_TAG(RESOURCE_ID,TAG_ID) VALUES($1,$2)`
	_, err := db.DB().Exec(sqlStatement, resourceID, tagID)
	if err != nil {
		log.Println(err)
	}
}

func addGithubDetails(db *gorm.DB, resourceID int, owner string, respositoryName string, path string) {
	sqlStatement := `INSERT INTO GITHUB_DETAIL(RESOURCE_ID,OWNER,REPOSITORY_NAME,PATH) VALUES($1,$2,$3,$4)`
	_, err := db.DB().Exec(sqlStatement, resourceID, owner, respositoryName, path)
	if err != nil {
		log.Println(err)
	}
}

func updateGithubYAMLDetails(db *gorm.DB, resourceID int, path string) {
	sqlStatement := `UPDATE GITHUB_DETAIL SET PATH=$1 WHERE RESOURCE_ID=$2`
	_, err := db.DB().Exec(sqlStatement, path, resourceID)
	if err != nil {
		log.Println(err)
	}
}

func updateGithubREADMEDetails(db *gorm.DB, resourceID int, path string) {
...
func addUserResource(db *gorm.DB, userID int, resourceID int) error {
	sqlStatement := `INSERT INTO USER_RESOURCE(RESOURCE_ID,USER_ID) VALUES($1,$2)`
	_, err := db.DB().Exec(sqlStatement, resourceID, userID)
	if err != nil {
		return err
	}
	return nil
...
func CheckSameResourceUpload(db *gorm.DB, userID int, name string) bool {
...
func GetAllResources(db *gorm.DB) []Resource {
	resources := []Resource{}
	sqlStatement := `SELECT * FROM RESOURCES ORDER BY ID`
	...
	sqlStatement = `SELECT ID FROM RESOURCES ORDER BY ID`
	rows, err = db.DB().Query(sqlStatement)
	if err != nil {
		log.Println(err)
	}
...
func GetResourceByID(db *gorm.DB, id int) Resource {
	resource := Resource{}
	...
	err := db.DB().QueryRow(sqlStatement, id).Scan(&resource.ID, &resource.Name, &resource.Type, &resource.Description, &resource.Downloads, &resource.Rating, &resource.Github, &resource.Tags, &resource.Verified)
	if err != nil {
		return Resource{}
	}
...
func GetTaskNameFromID(db *gorm.DB, taskID string) string {
	...
	var taskName string
	err = db.DB().QueryRow(sqlStatement, id).Scan(&taskName)
	if err != nil {
		return ""
	}
	return taskName
...
func IncrementDownloads(db *gorm.DB, taskID string) {
	id, err := strconv.Atoi(taskID)
	if err != nil {
		log.Println(err)
	}
...
func updateAverageRating(db *gorm.DB, resourceID int, rating float64) error {
	sqlStatement := `UPDATE RESOURCE SET RATING=$2 WHERE ID=$1`
	_, err := db.DB().Exec(sqlStatement, resourceID, rating)
	if err != nil {
		log.Println(err)
		return err
	}
...
func GetResourceIDFromName(db *gorm.DB, name string) (int, error) {
	sqlStatement := `SELECT ID FROM RESOURCE WHERE NAME=$1`
	var resourceID int
	err := db.DB().QueryRow(sqlStatement, name).Scan(&resourceID)
	if err != nil {
		log.Println(err)
		return 0, err
	}
...
func resourceExists(db *gorm.DB, resourceName string) bool {
	sqlStatement := `SELECT EXISTS(SELECT 1 FROM RESOURCE WHERE NAME=$1 AND VERIFIED=$2)`
	var exists bool
	err := db.DB().QueryRow(sqlStatement, resourceName, true).Scan(&exists)
	if err != nil {
		log.Println(err)
	}
	return exists
...
func DeleteResource(db *gorm.DB, resourceID int) error {
